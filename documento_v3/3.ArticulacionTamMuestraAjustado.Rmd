...
El tamaño de muestra obtenido en la región central y Huetar norte no se alcanza el tamaño de muestra necesario para el tamaño de muestra anual de pobreza extrema.

Consideramos un tamaño de muestra combinado para desocupación y pobreza extrema:

Para pobreza extrema: Para todas las regiones se conserva el tamaño de muestra de desocupación con excepción de las regiones Central y Huetar Norte que conservarán como tamaño de muestra el tamaño mínimo sugerido para pobreza extrema:
...
psuinsample_pobreza <- ifelse(tabla_indicadores_pobrezaDesocup_Region$agregacion %in%
                       ...
                       tabla_indicadores_pobrezaDesocup_Region$PSUinSample)
...
```{r}
tabla_Central_PE <- f_tablaMue_PobrezaExtrema(df_parametrostablas_pobrezaExtrema, "Region_Central",
...
df_pobrezaAnualNacional <- data.frame(agregacion = "Nacional", 
                                      PSUinSample = sum(df_pobrezaAnualRegion$PSUinSample),
                            cve = NA)
...
tabla_Nacional_PE <- f_tablaMue_PobrezaExtrema(df_parametrostablas_pobrezaExtrema,
                          "Nacional",
cve = df_pobrezaAnualNacional$cve / 100, m = 6)
```
...
También calculamos el tamaño de muestra para la desocupación:
